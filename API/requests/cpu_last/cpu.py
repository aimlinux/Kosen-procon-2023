...
def cpu(env,n):
        ...
        if env.f[0]=="C" and env.f[1]=="1":# C系
            if env.p==0:
                if n==env.shoku-2 and env.turn > env.maxturn-2:
                    print(env.turn,env.maxturn)
                    return 1,2
                if n==0 and env.turn > env.maxturn-10 and shy!=env.h//2-2:
                    print(env.turn,env.maxturn,n)
    ...
    if env.cp==3:
        if env.f=="A15":
            ac=[[17,17,11,11],[21,23,25,27],
                [18,16,12,10],[23,27,21,25],
                [15,13,15,13],[25,21,27,23],
                [27,27,21,21],[10,16,12,18],
                [25,23,25,23],[23,27,21,25],
                [11,11,17,17],[16,18,10,12],
                [21,21,27,27],[23,27,21,25],
                [25,23,25,23],[],
                [23,25,23,25],[],
                [16,18,10,12],[],
                [23,25,23,25],[],
                [27,27,21,21],[]]
            if env.maxturn-env.turn<len(ac):
                if ac[env.maxturn-env.turn]==[]:
                    env.c=[1,1,1,2]
                    env.cp=2
                    return cpu(env,n)
                else :
                    return ac[env.maxturn-env.turn][n]//10,ac[env.maxturn-env.turn][n]%10
            else:
                env.c=[1,1,1,2]
                env.cp=2
                return cpu(env,n)
    if env.cp==4:
            ...
            if ax >= 0 and ax < env.w and ay >= 0 and ay < env.h:
                if env.a[0][ay][ax]==400 and env.a[1+env.p][ay][ax]==0 and env.a[0][ay][ax]%100==0 and env.cpu_a[ay][ax]//1000!=env.turn*10+1:# and env.cpu_a[ay][ax]%10!=env.p+1:
                    ...
                    for j in [1,3,5,7]:
                        bx = ax+j % 3-1
                        by = ay+j//3-1
                        if bx >= 0 and bx < env.w and by >= 0 and by < env.h:
                            if env.a[0][by][bx]//100!=4 and env.a[1+env.p][by][bx]==0:
                                count+=1
                    print(count)
                    if count>=countmax:
                        countmax=count
        ...
        for i in [0,1,8,2,6,3,5,7]:# 城に移動
            ...
            ay = shy+i//3-1
            if ax >= 0 and ax < env.w and ay >= 0 and ay < env.h:
                if env.a[0][ay][ax]==400 and env.a[1+env.p][ay][ax]==0 and env.a[0][ay][ax]%100==0 and env.cpu_a[ay][ax]//1000!=env.turn*10+1:
                    print("城移動0")
                    bool1=True
                    count=0
                    for j in [1,3,5,7]:
        ...
        for i in [0,1,2,3,5,6,7,8]:
            ...
            ay = shy+i//3-1
            if ax >= 0 and ax < env.w and ay >= 0 and ay < env.h:
                if env.a[0][ay][ax]//100==4 and env.a[0][ay][ax]%100//10!=1+env.p and env.a[1+env.p][ay][ax]==0:
                    bool1=False
        if bool1:
            a1=[[0]*env.w for i in range(env.h)]
            a2=[[shy,shx]]
            ...
            while bool0:
                ...
                for i in a2:
                    for j in [0,1,2,3,5,6,7,8]:
                        ...
                        ay = i[0]+j//3-1
                        if ax >= 0 and ax < env.w and ay >= 0 and ay < env.h:
                            if a1[ay][ax]==0 and (j%2!=0 or env.a[2-env.p][ay][ax]<6) and env.cpu_a[ay][ax]//1000!=env.turn*10+1:
                                if env.a[0][ay][ax]%100//10!=1+env.p:
                                    bool2=True
                                    for l in range(env.shoku):
                                        if env.cpu_1[l]==[ay,ax] and l!=n:
                                            bool2=False
                                    if bool2:
                                        
                                        if env.a[0][ay][ax]%100!=30:
                                            a1[ay][ax]=k
                                            a3=a3+[[ay,ax]]
                                        if env.a[0][ay][ax]//100==4 and env.a[1+env.p][ay][ax]==0:
                                            bool0=False
                                            env.cpu_1[n]=a4=[ay,ax]
                a2=a3
            
            if bool1:
                for i in range(k-2):
                    for j in [0,1,2,3,5,6,7,8]:
                        if env.a[2-env.p][a4[0]][a4[1]]>5 and j%2==0:
                            continue
                        ax = a4[1]+j % 3-1
                        ay = a4[0]+j//3-1
                        if ax >= 0 and ax < env.w and ay >= 0 and ay < env.h:
                ...
                # if n==0:
                    ...
                    #     print(a1[i])
                    # print("\n")
                if env.a[2-env.p][ay][ax]>5:
                    print("解体２")
                    return 3,(ax-shx+1)+(ay-shy+1)*3
                if env.a[0][ay][ax]%100==0:
                    print("城移動２",(ax-shx+1)+(ay-shy+1)*3)
                    return 1,(ax-shx+1)+(ay-shy+1)*3
        for i in [1,3,5,7]:# 相手の城壁を解体
            ax = shx+i % 3-1
            ay = shy+i//3-1
