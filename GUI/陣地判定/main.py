...
def decj(p,area,aa):
    global a, point
    point[p] = 0
    for i in range(np.shape(area)[0]-1):
        a[p+1] = area[i+1] // (i+1)
...
def jinti(p,a,ax,ay,aa,area,n):
    ...
    # while(True):    
    #     if ax+1 < np.shape(aa)[1] -3:
    #         if a[0][ax+1][ay] == 0:
    #             jinti(p,a,ax+1,ay,aa,area,n)
        
    #     elif ay+1 < np.shape(aa)[0] -2:
    #         if a[0][1][ay+1] == 0:
    #             jinti(p,a,1,ay+1,aa,area,n)

    #     elif ax+1 == np.shape(aa)[1] and ay+1 == np.shape(aa)[0]:
    #         area[0] = np.zeros_like(aa)
    #         decj(p,area)
        
    #     if ax+1 < np.shape(aa)[0] -1:
    #         ax += 1
    #     else:
    #         ax = 1
    #         ay += 1
...
# def jinti(p,a,ax,ay,aa,area,sx,sy):
...
#                 if aa[i][j] == 0 :
#                     jinti(p,a,i-1,j,aa,area,sx,sy)
#         for i in range(aa.shape[0]):
#             for j in range (aa.shape[1]):
#                 if aa[i][j] == 1:
#                     a[p+1][i][j] += 1
#                     cvs.create_rectangle(12+ax*w_width, 12+ay*h_width,9+(
#                                          ax+1)*w_width,9+(ay+1)*h_width, fill=jin[p], width=0)
    

#     elif aa[ax][ay+1] == 0:
#         if ax+1 == sx and ay == sy:
#             aa = aa + area
#             for i in range(np.shape(aa)[0]):
#                 for j in range(np.shape(aa)[1]):
#                     if aa[i][j] == 0 :
...
#                         jinti(p,a,i-1,j,aa,area,sx,sy) 
#         else:area[ax][ay+1] = 1
#         jinti(p,a,ax+1,ay+1,aa,area,sx,sy)
#     elif aa[ax][ay+1] == -1:
#         aa =  aa - area
#         # area = np.zeros_like(aa)
#         for i in range(np.shape(aa)[0]):
...
#                         jinti(p,a,i-1,j,aa,area,sx,sy) 
#         else:area[ax-1][ay] = 1
#         jinti(p,a,ax-1,ay,aa,area,sx,sy)
#     elif aa[ax-1][ay] == -1:
#         aa =  aa - area
#         area = np.zeros_like(aa)
#         for i in range(np.shape(aa)[0]):
...
#                         jinti(p,a,i-1,j,aa,area,sx,sy) 
#         else:area[ax][ay-1] = 1
#         jinti(p,a,ax,ay-1,aa,area,sx,sy)
#     elif aa[ax][ay-1] == -1:
#         aa =  aa - area
#         area = np.zeros_like(aa)
#         for i in range(np.shape(aa)[0]):
