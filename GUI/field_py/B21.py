...
# 職人aを配置
sh0_x = [6,6,7,13,14,14] #駒の配置は左上から右下の順番で
sh0_y = [3,17,10,10,3,17]
i = 0
while i < shoku:
    x = sh0_x[i]
    y = sh0_y[i]
    if a[0][y][x] == 0:
        ...
        i += 1
# 職人bを配置
sh1_x = [3,3,10,10,17,17]
sh1_y = [6,14,7,13,6,14]
i = 0
while i < shoku:
    x = sh1_x[i]
    y = sh1_y[i]
    if a[0][y][x] == 0:
        ...
        i += 1
# 池を配置
ike_x = [1,1,2,2,2,3,3,3,3,3,4,4,5,5,5,5,6,6,6,7,7,7,7,8,8,8,8,9,9,10,10,10,10,10,10,10,11,11,12,12,12,12,13,13,13,13,14,14,14,15,15,15,15,16,16,17,17,17,17,17,18,18,18,19,19]
ike_y = [9,11,3,10,17,2,4,10,16,18,3,17,7,8,12,13,7,10,13,5,6,14,15,5,8,12,15,1,19,2,3,6,10,14,17,18,1,19,5,8,12,15,5,6,14,15,7,10,13,7,8,12,13,3,17,2,4,10,16,18,3,10,17,9,11]
i = 0
ik = [[0, 0]]*ike
while i < ike:
    x = ike_x[i]
    ...
    if a[0][y][x] == 0:
        ...
        i += 1
# 城を配置
si_x = [1,1,1,1,1,3,3,6,6,6,6,9,9,10,10,11,11,14,14,14,14,17,17,19,19,19,19,19]
si_y = [1,6,10,14,19,3,17,1,6,14,19,9,11,1,19,9,11,1,6,14,19,3,17,1,6,10,14,19]
i = 0
si = [[0, 0]]*siro
while i < siro:
    x = si_x[i]
